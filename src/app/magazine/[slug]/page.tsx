    ...
    <div>
      ...
      <div className="relative h-[60vh] md:h-[70vh]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16 max-w-4xl">
          ...
          <p className="text-gray-300 text-sm mb-4">{article.subtitle}</p>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <h3 className="font-serif text-2xl text-white mb-10">관련 아티클</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {related.map((r) => (
            <Link key={r.id} href={`/magazine/${r.id}`} className="group">
              ...
              <p className="text-xs tracking-[0.15em] uppercase text-gold mt-4">{r.categoryLabel}</p>
              <p className="font-serif text-base text-gray-200 mt-1 group-hover:text-gold transition-colors">{r.title}</p>
            </Link>
          ))}
        </div>
      </section>
