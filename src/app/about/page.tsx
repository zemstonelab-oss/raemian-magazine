    ...
    <div className="pt-28 pb-24 bg-white">
      ...
      <div className="max-w-4xl mx-auto px-6 text-center mb-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <p className="font-heading text-xs tracking-[0.3em] uppercase text-teal mb-3">Brand Story</p>
          <h1 className="font-heading text-4xl md:text-5xl font-light text-gray-900 mb-8">About Raemian</h1>
          ...
          {[
            ...
            <AnimatedSection key={item.title} delay={i * 0.15}>
              <div className="text-center">
                <h3 className="font-heading text-2xl text-teal mb-4">{item.title}</h3>
      ...
      <section className="max-w-3xl mx-auto px-6">
        <AnimatedSection>
          <p className="font-heading text-xs tracking-[0.3em] uppercase text-teal mb-3">History</p>
          <h2 className="font-serif text-3xl text-gray-900 mb-16">연혁</h2>
        </AnimatedSection>
          ...
          {timeline.map((item, i) => (
            <AnimatedSection key={item.year} delay={i * 0.1}>
              <div className={`relative flex items-start gap-8 mb-12 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                ...
                <div className="relative z-10 w-8 h-8 rounded-full bg-white border-2 border-teal flex items-center justify-center shrink-0">
                  <div className="w-2 h-2 rounded-full bg-teal" />
                </div>
                <div className="flex-1">
                  <span className="text-teal font-serif text-lg">{item.year}</span>
                  <h3 className="text-gray-900 font-medium mt-1">{item.event}</h3>
